...
| **Base URL** | `https://api.look2eye.com` |
| **OpenAI Compatible API Key** | 你的 Look2Eye API Key |
| **Model ID** | `openai/gpt-4.1`（或其他模型） |

![填写配置信息](../assets/cline/03-填写配置信息.webp)

> ℹ️ Cline 也支持 Anthropic 和 Gemini 协议，对应的 Base URL 分别为 `https://api.look2eye.com` 和 `https://api.look2eye.com`。
...
切换 API Provider 为 Anthropic，Base URL 改为 `https://api.look2eye.com/anthropic`，可获得完整的 Tool Use 支持。
