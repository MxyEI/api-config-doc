...
Complete [Configure Model Provider](model-provider.md) first, then run the one-line command to enable the Codex Responses protocol format and WebSocket connection. The script only updates connection capability. It does not read, save, or overwrite your API key.
