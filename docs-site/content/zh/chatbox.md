...
| API 模式 | API 主机 | 示例模型 |
| --- | --- | --- |
| OpenAI API 兼容 | `https://api.api.look2eye.com/v1` | `openai/gpt-4.1`、`openai/gpt-5.3-chat` |
| OpenAI Responses API 兼容 | `https://api.api.look2eye.com/v1` | `openai/gpt-4.1`、`openai/gpt-5.3-chat` |
| Anthropic Claude API 兼容 | `https://api.api.look2eye.com/anthropic/v1` | `anthropic/claude-sonnet-4.6`、`anthropic/claude-opus-4.6` |
| Google Gemini API 兼容 | `https://api.api.look2eye.com/gemini` | `gemini-2.5-flash`、`gemini-3.1-pro-preview` |
...
-   **API 主机**：填写对应的 API 地址（如 Claude 为 `https://api.api.look2eye.com/anthropic/v1`）
...
-   **模型 ID**：从 [Look2Eye 模型广场](https://api.look2eye.com/models)  复制模型 ID 填入
-   **模型类型**：选择 **聊天**
...
1.  确认 API Key 从 [Look2Eye 控制台](https://api.look2eye.com/console/api-keys)  完整复制，无多余空格
2.  确认 API 主机填写正确
3.  确认网络连接正常
