---
title: "Claude Coworks 配置"
description: "在 Claude Coworks 中启用第三方推理 Gateway，并通过 Look2Eye 的 Anthropic 协议调用 Claude 系列模型。"
---
...
![开启开发者模式](/assets/claude-coworks/01-enable-developer-mode.webp)

> ℹ️ 开启开发者模式后，菜单栏会出现 **Developer** 菜单。

### 第 2 步：打开第三方推理配置

在菜单栏点击 **Developer → Configure Third-Party Inference…**，打开配置面板。

![打开配置面板](/assets/claude-coworks/02-open-third-party-inference.webp)

### 第 3 步：选择 Gateway

在 **Connection** 页面选中 **Gateway（Anthropic-compatible）**。

![选择 Gateway](/assets/claude-coworks/03-select-gateway.webp)
...
![填写凭据并应用](/assets/claude-coworks/04-gateway-credentials.webp)
